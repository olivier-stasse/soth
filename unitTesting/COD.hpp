...
namespace soth
{
  ...
  struct COD
  {
    ...
    COD( unsigned int computationOptions = 0)
    {
	...
	{
	  if(fabs(QR(rank-1,rank-1))>EPSILON)
	    break;
	}
      return rank;
    }
    ...
    TriangularView<Block<MatrixXd>,Lower> matrixL() { return L.topRows(rank).triangularView<Eigen::Lower>(); }
